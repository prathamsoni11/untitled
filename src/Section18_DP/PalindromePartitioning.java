...
public class PalindromePartitioning {
    ...
    public static boolean[][] palindromeInfo(String s){
        boolean[][] isStrongPalindrome = new boolean[s.length()][s.length()];
        for (int i = 0 ; i < isStrongPalindrome.length ; i++){
            Arrays.fill(isStrongPalindrome[i],true);
        }
        for (int row = s.length()-2 ; row>=0 ; row--){
            for (int col = row+1 ; col < s.length() ; col++){
                if (s.charAt(row) == s.charAt(col)){
                    isStrongPalindrome[row][col] = isStrongPalindrome[row+1][col-1];
                }else {
                    isStrongPalindrome[row][col] = false;
                }
            }
        }
        return isStrongPalindrome;
    }
}
